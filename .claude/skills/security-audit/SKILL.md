...
After collecting answers, delegate the codebase scan to the **security-auditor agent** (`.claude/agents/security-auditor.md`):

```
Use the Agent tool with subagent_type="general-purpose" and model="haiku".
Pass the collected context (app type, attack surfaces, sensitive data, deployment) as the prompt.
The agent scans the codebase and returns a structured Security Audit Report.
```

The agent handles: secret scan, dependency check, auth review, input handling, configuration, logging. It returns a severity-ranked findings report with actionable fixes.
...
For DevSecOps pipeline integration, STRIDE workflow, attack tree analysis, and compliance mapping, read `security-audit/advanced-techniques.md`.
