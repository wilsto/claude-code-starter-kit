...
│   ├── commands/      → slash commands (/commit, /tdd, /review, /test-runner)
│   ├── agents/        → custom agent definitions (test-runner, security-auditor, tech-debt-auditor)
...
- **TodoWrite**: track progress on single-thread tasks (subtask decomposition, status updates)
- **Task tool**: coordinate multi-agent workflows (spawn subagents, parallel execution, cross-agent context)
- Both tools are valid — use Task tool when delegating to agents (test-runner, security-auditor, etc.)
